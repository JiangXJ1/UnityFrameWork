...
func GetLuaCellValue(cellValue string, cellType string) (string, error) {
	fixCellType := strings.TrimSpace(cellType)
	if len(fixCellType) == 0 {
		//panic("字段类型不能为空字符串！")
		return "", errors.New("字段类型不能为空字符串")
	}
	fixCellType = strings.ToLower(fixCellType)
	ft := common.GetFieldType(fixCellType)
	var buffer bytes.Buffer
	switch ft {
	case common.FieldType_Int32, common.FieldType_Float, common.FieldType_UInt32:
		return GetNumber(cellValue), nil
		break
