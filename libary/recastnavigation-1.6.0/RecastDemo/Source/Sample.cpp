...
bool Sample::SaveAs(const char* path)
{
	return false;
}
...
const std::string Sample::getTypeName()
{
	return std::string();
}

std::string Sample::getSavePath() 
{
	if (m_geom)
	{
		return m_geom->getMesh()->getOutputFilePath() + getTypeName() + ".bin";
	}
	return getTypeName();
}
