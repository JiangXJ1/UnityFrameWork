...
NavMeshTesterTool::NavMeshTesterTool() :
	...
	m_straightPathOptimizedLevel(0),
	m_startRef(0),
	m_endRef(0),
	m_npolys(0),
	m_nstraightPath(0),
...
void NavMeshTesterTool::handleMenu()
{
	...
	{
		imguiIndent();
		imguiLabel("Straight Path Optimized Level");
		if (imguiCheck("None", m_straightPathOptimizedLevel == 0))
		{
			m_straightPathOptimizedLevel = 0;
			recalc();
		}
		if (imguiCheck("Line Optimized", m_straightPathOptimizedLevel == 1))
		{
			m_straightPathOptimizedLevel = 1;
			recalc();
		}
		if (imguiCheck("Cross Area Optimized", m_straightPathOptimizedLevel == 2))
		{
			m_straightPathOptimizedLevel = 2;
			recalc();
		}
...
void NavMeshTesterTool::recalcWithRegion()
{
	std::string path = m_sample->getSavePath();
	...
	if (buffer)
	{
		...
		NavMeshRegion* region = new NavMeshRegion(12123);
		region->InitData(buffer, size);

		//region->SetLogFilePath("D:\\cdc03\\client\\libary\\recastnavigation-1.6.0\\RecastDll\\NavgationLOG_CPP.txt");

		free(buffer);

		float halfExtents[] = { 10, 15, 10 };
		m_nstraightPath = 0;
		switch (m_straightPathOptimizedLevel)
		{
		case 0:
			m_nstraightPath = region->FindPath(halfExtents, m_spos, m_epos, m_straightPath);
			break;
		case 1:
			m_nstraightPath = region->FindPathLineOptimized(halfExtents, m_spos, m_epos, m_straightPath);
			break;
		case 2:
			//m_spos[0]= 569.21f;
			//m_spos[1]= 0;
			...
			m_nstraightPath = region->FindPathCrossAreaOptimized(halfExtents, m_spos, m_epos, m_straightPath);
			break;
		}
