...
void Sample_TempObstacles::handleMeshChanged(class InputGeom* geom)
{
	...
	const BuildSettings* buildSetting = geom->getBuildSettings();
	if (buildSetting) 
	{
		m_tileSize = buildSetting->tileSize;
	}
...
bool Sample_TempObstacles::SaveAs(const char* path)
{
