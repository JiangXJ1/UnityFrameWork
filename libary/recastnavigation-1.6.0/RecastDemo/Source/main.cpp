...
{
	{ createSolo, "Solo_Mesh" },
	{ createTile, "Tile_Mesh" },
	{ createTempObstacle, "Temp_Obstacles" },
};
...
int DoSDLWindow() 
{
	...
	//string sampleName = "Temp_Obstacles";
	...
	InputGeom* geom = 0;
	Sample* sample = 0;
	//Sample* sample = new Sample_TempObstacles();
...
int main(int argc, char** argv)
{
	...
	else
	{
		...
		BuildContext ctx;
		geom = new InputGeom;
		geom->load(&ctx, meshPath);

		if (savePath != nullptr)
			geom->setOutputFilePath(savePath);
		if (setting != nullptr) 
		{
			geom->setSetting(setting);
		}

		for (int i = 0; i < g_nsamples; ++i) 
		{
			auto pSample = g_samples[i];
			if (strcmp(modeType, pSample.name.c_str()) == 0)
			{
				sample = pSample.create();
